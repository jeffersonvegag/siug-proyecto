...
<div class="section-body">
  <div class="row">
    <div class="col-12">
      <div class="card">
        <h2 class="mb-4 card-header bg-dark py-6 card-label text-white fw-bolder fs-2">Formulario para editar el convenio marco</h2>
        <div class="card-body">


          <form id="form-editar" method="POST" enctype="multipart/form-data" action="?c=DocumentosGenerados&m=actualizarMarco&id=<?= urlencode($id_doc_gen) ?>">
            ...
            <div class="row">
              ...
              <!-- Vista previa: Título del convenio (CONVENIO MARCO) -->
              <div class="col-12 mb-3">
                <label class="form-label">
                  <strong>Convenio Marco (Vista Previa):</strong>
                </label>
                <div
                  id="preview_convenio_marco"
              ...
              <!-- Sección 7: COORDINACIÓN (o ADMINISTRACIÓN).  -->
              <div class="col-12 mb-4">
                <h4>CLÁUSULA SÉPTIMA: COORDINACIÓN (o ADMINISTRACIÓN).</h4>
                <div class="mb-3">
                  <label for="BLOQUE_ENRIQUECIDO3">Cláusula de COORDINACIÓN (o ADMINISTRACIÓN). - </label>
                  <textarea id="BLOQUE_ENRIQUECIDO3" name="BLOQUE_ENRIQUECIDO3" class="form-control usar-summernote" rows="4"> </textarea>
                  <small class="form-text text-muted">En esta cláusula se podría establecer de forma directa los delegados de las partes, pero solo en el caso que ambas partes los deleguen, en el caso de la Universidad de Guayaquil esta delegación será designada con conocimiento y aprobación del Rector, el delegado deberá ser un funcionario de la Universidad de Guayaquil, que cuente con las capacidades y cualidades necesarias para el desarrollo del convenio en relación a la materia del mismo.</small>
                </div>
              </div>
              ...
              <!-- Sección 8: Proyecto y Representantes -->
              <div class="col-12 mb-4">
                <h4>CLÁUSULA DÉCIMA SÉPTIMA: ACEPTACIÓN</h4>
                <div class="row">

                  <div class="col-md-6 mb-3">
                    <label for="FECHA_ACEPTACION_SIN_FORMATO">Fecha</label>
                    <input type="date" id="FECHA_ACEPTACION_SIN_FORMATO" name="FECHA_ACEPTACION_SIN_FORMATO" class="form-control">
                  </div>

                  <div class="col-md-6 mb-3">
                    <label for="FECHA_ACEPTACION_CON_FORMATO">Fecha de Aceptación</label>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (btnSalir) {
        btnSalir.addEventListener('click', function(event) {
            event.preventDefault(); // Prevenimos la navegación
            const url = this.href;
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = url; // Si confirma, redirigimos
                }
            });
        });
    }
    ...
    if (btnGuardar) {
        btnGuardar.addEventListener('click', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    btnGuardar.closest('form').submit(); // Si confirma, enviamos el formulario
                }
            });
        });
    }
    ...
    if (btnDescargar) {
        btnDescargar.addEventListener('click', function(event) {
            event.preventDefault(); // Prevenimos que siga el enlace
            const url = this.href;
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.open(url, '_blank'); // Si confirma, abrimos el enlace en nueva pestaña
                }
            });
        });
    }
...
<!-- 1 -->
<!-- Encabezado inicial del convenio marco -->
<script>
  function actualizarParrafoInicialConvenioMarco() {
    const RAZON_SOCIAL_MAYUS = escapeHtml(document.getElementById('RAZON_SOCIAL_MAYUS').value || '______');

    const textoConvenio = `CONVENIO MARCO DE COOPERACIÓN INTERINSTITUCIONAL ENTRE LA UNIVERSIDAD DE GUAYAQUIL Y <strong>${RAZON_SOCIAL_MAYUS}</strong>.`;

    document.getElementById('preview_convenio_marco').innerHTML = textoConvenio;
  }
</script>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    sincronizarMayus('RAZON_SOCIAL', 'RAZON_SOCIAL_MAYUS', actualizarParrafoInicialConvenioMarco);
    ...
    ['RAZON_SOCIAL_MAYUS'].forEach(id => {
      ...
      if (el) el.addEventListener('input', actualizarParrafoInicialConvenioMarco);
      if (el) el.addEventListener('change', actualizarParrafoInicialConvenioMarco);
    });

    actualizarParrafoInicialConvenioMarco();
  });
</script>
